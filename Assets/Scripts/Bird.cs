...
public class Bird : MonoBehaviour
{
    ...
    private void Update()
    {
        if (ManagerGame.inst.gameMode != 1) return; // Game Play가 아니면 터치 대응 x
    ...
    void BirdBlink()
    {
        ...
        if(fBlink < 0f) // 껌뻑거리는 시간 종료
        {
            ...
            if (ManagerGame.inst.Life == 0) _coll.isTrigger = false; 
        }
    }
    ...
    {
        if (state)
            GameObject.Find("GameManager").GetComponent<ManagerGame>().CheckBirdBlinking = true;
        else
            GameObject.Find("GameManager").GetComponent<ManagerGame>().CheckBirdBlinking = false;
    }
}
